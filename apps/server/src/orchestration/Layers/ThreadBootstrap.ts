/**
 * ThreadBootstrap - Live implementation of bootstrapped turn starts.
 *
 * Moved out of the WebSocket handler so the MCP thread toolkit can reuse it.
 * The bootstrap creates the thread, prepares its worktree, runs the project
 * setup script, then dispatches the turn start with the `bootstrap` block
 * stripped. Every failure path rolls the created thread back.
 *
...
function projectSetupScriptCompatibilityDetail(
  ...
  switch (error._tag) {
    ...
    default: {
      ...
      throw new Error(`Unhandled compatibility error: ${String(unexpected)}`);
    }
  }
}
...
const make = Effect.gen(function* () {
  ...
  const dispatchTurnStart: ThreadBootstrapShape["dispatchTurnStart"] = (command, options) =>
    Effect.gen(function* () {
      // Every command the bootstrap emits carries the caller's origin, so a
      // connection's thread creation stays attributed to that client and a
      // server-side caller stays unattributed.
  ...
  return { dispatchTurnStart } satisfies ThreadBootstrapShape;
});
